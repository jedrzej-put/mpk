...
def test_all_stops(stops_controller):
    ...
    assert stops_controller.all_stops[0] == {
        "stop_id": "15",
        "stop_code": "12525",
        "stop_name": "Metalowców",
        "stop_lat": "51.13382609",
        "stop_lon": "16.95673512",
    }
...
def test_nearest_stops_to_up_count(stops_controller):
    stops = stops_controller.all_stops[:]
    lat = "51.13382609"
    lon = "16.95673511"
    nearest_stops = stops_controller.nearest_stops_to_up(
        lat=lat, lon=lon, age=20, count=2, stops=stops
    )
    LOGGER.info(nearest_stops)
    assert nearest_stops[0]["stop_id"] == "15"  # "Metalowców",
    assert nearest_stops[1]["stop_id"] == "1577"  # "Metalowców",
